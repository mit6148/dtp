...
function searchEvents(event) {
    ...
    .done(function(res) {
        ...
        $('#eventsTable').show();
        if (res.length == 0){
            $('#events').append('<tr><td id="centerCell" colspan="' + ((logged_in) ? '8' : '7') + '"><p>No results returned :(</p><button class="ui blue center floated icon submit button" onclick="createEventFromSearch();">Create Event</button></td></tr>');
        } else {
