...
function populateInvitations(invitations) {
	...
	if (invitations.length === 0) {
		...
		$('#invitationsBody').html('<div id="segBody"></div>');
	}
...
function addToInvitationsTable(invitation) {
	...
	str += '<div class="ui clearing segment">Invitation from <b>' + invitation.inviter.name + '</b> for <b>' + invitation.event.course + '</b>: <b>' + invitation.event.assignment + '</b><div class="ui right floated red button" onclick="dismissInvitation(' + invitation.event_id + ')">Dismiss</div><div class="ui right floated green button invitationButton" onclick="acceptInvitation(' + invitation.event_id + ')">Accept</div></div><div class="ui segments">';
