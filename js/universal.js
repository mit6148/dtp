...
function message(div_id, messageType, headerText, bodyText) {
  $('#'+div_id).prepend('<div><div class="fixed message ui ' + messageType + ' floating compact message"><i onclick="$(this).hide()"class="close icon"></i><div class="header">' + headerText + '</div><p>' + bodyText + '</p></div></div>');
  setTimeout(function() {
    $("#" + div_id + " > *").hide();
  }, 4000);
}
