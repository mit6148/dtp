...
function eventButton(id, signed_up) {
    if (signed_up) {
        $('#' + id).html('<button class="ui black right labeled icon cancel button" id="cancel_' + id + '" onclick="cancel_signup(' + id + ')">Cancel<i class="remove icon"></i></button>');
    } else {
        $('#' + id).html('<button class="ui blue right labeled icon submit button" id="signup_' + id + '" onclick="signup(' + id + ')">Signup<i class="add icon"></i></button>');
    }
}