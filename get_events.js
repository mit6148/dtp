...
function parseEvent(event) {
    ...
    $('#events').append(('<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td>' + ((logged_in) ? '<td>{7}</td>' : '') + '</tr>').format(event.course, event.assignment, event.location, start_date[0] + " " + start_date[1] + " " + start_date[2], parseTime(start_time), parseTime(end_time), (event.owner_sub == sub) ? 'You' : ('<a href="mailto:' + event.owner_email + '">' + event.owner_name + '</a>'), (event.is_signed_up ? '<button class="ui black right labeled icon cancel button" id="cancel_' + event.id + '" onclick="cancel_signup(' + event.id + ')">Cancel<i class="remove icon"></i></button>' :'<button class="ui blue right labeled icon submit button" id="signup_' + event.id + '" onclick="signup(' + event.id + ')">Signup<i class="add icon"></i></button>')));
}
